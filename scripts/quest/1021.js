...
function start(mode, type, selection){
	...
	if (status == 0)
		...
		qm.showInfo("USING_ITEMS_ADVENTURER");
        qm.dispose();
    }
}
...
function end(mode, type, selection){
	...
	if (status == 0)
        ...
        if(qm.isQuestCompleted(1021))
            qm.sendMessage("PINK_TEXT", "Unknown Error");
        else if(qm.canHold(2010000) && qm.canHold(2010009)){
            qm.getClient().getCharacter().giveExp(10);
            qm.giveItem(2010000, 3);
            qm.giveItem(2010009, 3);
            qm.forceCompleteQuest();
        }else
            qm.dropMessage("PINK_TEXT", "Your inventory is full");
        qm.dispose();
    }
}