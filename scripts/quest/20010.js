...
function start(mode, type, selection){
	...
	if(mode == -1){
		qm.sendOk("Whoa, whoa! Are you really declining my offer? Well, you'll be able to #blevel-up quicker #kwith our help, so let me know if you change your mind. Even if you've declined a Quest, you can receive the Quest again if you just come and talk to me.");
        qm.dispose();
	} else {
		statusUpdate(mode);
		...
		if(status == 0){
           ...
           if(qm.canHold(2000022) && qm.canHold(2000023)){
				if(!qm.isQuestCompleted(21010)) {		
					qm.giveItem(2000020, 5);
					qm.giveItem(2000021, 5);
					qm.getCharacter().giveExp(5);
				}
				qm.sendGuideHint(3, 5000);
