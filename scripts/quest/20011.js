...
function start(mode, type, selection) {
	...
	if (status == 2) {
	    qm.sendOk("You don't want to? It's not even that hard, and you'll receive special equipment as a reward! Well, give it some thought and let me know if you change your mind.");
	    qm.dispose();
	    return;
	}
	status--;
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
		...
		qm.giveItem(1002869, 1);
		qm.giveItem(1052177, 1);
		qm.forceCompleteQuest();
		qm.getCharacter().giveExp(30);
		qm.sendGuideHint(6, 5000);
		qm.dispose();
    }
}