...
function start(mode, type, selection) {
	...
	if (status == 2) {
	    qm.sendOk("Regular Attacks are basic skills that are easy to use. It is important to remember that real hunting is done using your Skills. I suggest you reconsider.");
	    qm.dispose();
	    return;
	}
		status--;
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
		...
		qm.giveItem(4000483, -1);
		qm.forceCompleteQuest();
		qm.getCharacter().giveExp(40);
		qm.dispose();
    }
}